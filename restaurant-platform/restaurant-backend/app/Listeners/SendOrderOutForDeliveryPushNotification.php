 ...
 */
final class SendOrderOutForDeliveryPushNotification
{
    public function handle(OrderOutForDelivery $event): void
    {
        ...
        SendCustomerPushNotificationJob::dispatch(
            ...
            title: __('notifications.order_out_for_delivery.title'),
            body: __('notifications.order_out_for_delivery.body', ['number' => $order->order_number]),
            data: [
                'order_id' => $order->id,
                'order_number' => $order->order_number,
                'status' => $order->status->value,
            ],
            idempotencyKey: sprintf('push:%s:%d', OrderOutForDelivery::class, $order->id),
        );
    }
}
