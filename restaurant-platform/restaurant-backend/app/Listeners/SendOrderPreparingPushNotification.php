 ...
 */
final class SendOrderPreparingPushNotification
{
    public function handle(OrderPreparing $event): void
    {
        ...
        SendCustomerPushNotificationJob::dispatch(
            ...
            title: __('notifications.order_preparing.title'),
            body: __('notifications.order_preparing.body', ['number' => $order->order_number]),
            data: [
                'order_id' => $order->id,
                'order_number' => $order->order_number,
                'status' => $order->status->value,
            ],
            idempotencyKey: sprintf('push:%s:%d', OrderPreparing::class, $order->id),
        );
    }
}
